...
const DATA_CACHE_KEY = "easyScanData";
const DATA_CACHE_TIME_KEY = "easyScanDataTime";
const DATA_REFRESH_INTERVAL = 12 * 60 * 60 * 1000;
...
document.getElementById("refreshDataButton").addEventListener("click", () => {
  ...
  backgroundUpdateSheet();
});


async function backgroundUpdateSheet() {
  try {
    ...
    const oldData = localStorage.getItem(DATA_CACHE_KEY) || "";
    const newData = JSON.stringify(freshData);

    if (oldData !== newData) {
      data = freshData;
      localStorage.setItem(DATA_CACHE_KEY, newData);
      localStorage.setItem(DATA_CACHE_TIME_KEY, Date.now().toString());
